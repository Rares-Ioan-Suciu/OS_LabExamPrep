...
// 6. Implement a basic mv-like command that moves the contents of one file to another.
...
int main(const int argc, const char **argv) {
    ...
    remove(argv[1]); // after copying we remove the original file, only difference from copy
    if (close(fd_des) == -1) {
